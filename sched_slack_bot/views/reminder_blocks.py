...
from slack_sdk.models.blocks import ConfirmObject, Block, SectionBlock, MarkdownTextObject, ButtonElement
...
def get_reminder_blocks(reminder: Reminder) -> List[Block]:
    return [
        SectionBlock(
            text=MarkdownTextObject(text=get_reminder_text(reminder)),
            block_id=reminder.schedule_id,
            accessory=ButtonElement(
                action_id=SKIP_CURRENT_MEMBER_ACTION_ID, text="Skip", confirm=_get_confirm_object_for_skip(reminder=reminder)
            ),
        )
    ]
...
def get_skip_blocks(reminder: Reminder) -> List[Block]:
    return [
        SectionBlock(
            text=MarkdownTextObject(text=get_skip_text(reminder)),
            block_id=reminder.schedule_id,
            accessory=ButtonElement(
                action_id=SKIP_CURRENT_MEMBER_ACTION_ID, text="Skip", confirm=_get_confirm_object_for_skip(reminder=reminder)
            ),
...
def get_reminder_text(reminder: Reminder) -> str:
    text = (
        f"Hey <@{reminder.user_id_to_notify}>, you are responsible for"
...
def get_skip_text(reminder: Reminder) -> str:
    text = (
        f"Skipped to <@{reminder.user_id_to_notify}>: you are responsible for"
